...
Инструмент записи долгосрочной памяти для активного использования в режиме tool loop / agent.
...
- Записывает долгосрочную память только когда пользователь явно просит что-то запомнить, либо когда Agent выявляет устойчивые предпочтения, долгосрочные факты, договорённости, сведения об идентичности или долгоживущий контекст проекта.
- При записи всегда используются текущий `event.unified_msg_origin` и текущий ID персоны, что соответствует модели принадлежности автоматической суммаризации.
- Перед сохранением повторно используется `MemoryProcessor.build_memory_from_structured_data()`, чтобы создать ту же структуру хранения, что и у автоматических суммаризаций.
- Инструмент не принимает пользовательские `session_id`, `persona_id` или произвольные metadata, чтобы Agent не записывал память в неверную область и не загрязнял схему хранения.

#### Входные параметры

- `memory: str` — Краткая фактическая долгосрочная память для сохранения. Не копируйте весь диалог.
- `topics: list[str] = []` — Необязательные тематические теги, сохраняется до 5.
- `key_facts: list[str] = []` — Необязательные ключевые факты, сохраняется до 5.
- `sentiment: str = "neutral"` — Метка тональности. Поддерживаются `positive`, `neutral` и `negative`.
- `importance: float = 0.7` — Оценка важности, нормализуется `MemoryProcessor` до диапазона `0.0-1.0`.
- `reason: str = ""` — Необязательная причина записи памяти, сохраняется только как исходная metadata.

#### Возвращаемый результат

Возвращает JSON-текст, содержащий следующие поля:

- `memorized`: Успешна ли запись
- `id`: ID новой памяти
- `content`: Содержимое памяти после стандартного форматирования
- `importance`: Нормализованная важность
